node {
    ...
    stage("Display output"){ 
	    ...
	    def out_data = """{"results":"${currentBuild.currentResult}","job_id":"${env.BUILD_NUMBER}"}"""
	    writeFile (file: 'output_file.json', text: out_data)	
	    sh "/google-cloud-sdk/bin/bq load --source_format NEWLINE_DELIMITED_JSON dataset1.output output_file.json results:string,job_id:integer"
    }
}
